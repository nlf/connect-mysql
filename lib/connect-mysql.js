...
module.exports = function (connect) {
    ...
    function MySQLStore(options) {
        ...
        this.mysql = this.client.useDatabase(this.database);
    }
    ...
    MySQLStore.prototype.get = function (sid, callback) {
        try {
            this.mysql.query('SELECT sessions.session, sessions.expires WHERE sessions.sid = ?', [sid], function (err, result) {
    ...
    MySQLStore.prototype.set = function (sid, session, callback) {
        ...
        this.mysql.query('INSERT INTO sessions(sid, expires, session) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE expires=?, session=?', [sid, ttl, session, ttl, session], function (err, result) {
            callback(err);
        });
    };
