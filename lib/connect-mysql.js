module.exports = function (connect) {
    ...
    function MySQLStore(options) {
        ...
        self.mysql.query('CREATE TABLE IF NOT EXISTS `sessions` (`sid` VARCHAR(255) NOT NULL, `session` TEXT NOT NULL, `expires` INT, PRIMARY KEY (`sid`) )', function (err) {
            if (err) throw err;
            self.mysql.query('CREATE EVENT `sess_cleanup` ON SCHEDULE EVERY 15 MINUTE DO DELETE FROM `sessions` WHERE `expires` < UNIX_TIMESTAMP()');
        });
    }
    ...
    MySQLStore.prototype.set = function (sid, session, callback) {
        ...
        if (typeof session.cookie.maxAge == 'number') {
            expires = (new Date().getTime() / 1000) + session.cookie.maxAge;
        }
