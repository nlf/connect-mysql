...
=======
    function encryptData(plaintext){
        var hmac = digest.call(this.secret, plaintext);
        ...
        var ct = encrypt.call(this.secret, JSON.stringify(obj));

        return ct;
    }

    function decryptData(ciphertext){
        var pt = decrypt.call(this.secret, ciphertext);
        var obj = JSON.parse(pt);
        var hmac = digest.call(this.secret, JSON.stringify(obj.pt));
    ...
    function encrypt(key, pt, algo) {
        ...
        return ct.join('');
>>>>>>> Crypto refactor to protect computed HMAC
    }
  });
        ...
        function(err, result) {
          if (result && result[0] && result[0].session) {
            try {
              ...
              if (secret) {
                session = JSON.parse(result[0].session, secret, this.algo);
              } else {
                session = JSON.parse(result[0].session);
              }
